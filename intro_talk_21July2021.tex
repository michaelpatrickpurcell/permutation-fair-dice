...
Notice that for a set of $n$ $d$-sided dice to be permutation fair,  the number of possible outcomes must be divisible by the number of permutations on $n$ elements.

\vfill

That is, we must have $n! \ \vert \ d^n$.

\vfill

So, for any set of five $d$-sided permutation fair dice we must have $ 30 \ \vert \ d$.
...
We can use the string representation of a set of dice to compute the probabilities of various outcomes when the dice are rolled.

\vfill

In particular, we can compute the number of ways that each permutation of the dice can occur when the dice are rolled.
